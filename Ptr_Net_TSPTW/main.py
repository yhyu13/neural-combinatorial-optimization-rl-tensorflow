    ...
    with tf.Session() as sess:
        ...
        # Inference mode
            ...
            for file_name in dataset:
                ...
                # Sample solutions
                permutations, reward, circuit_length, delay, delivery_time, attending, pointing = sess.run([actor.positions, actor.reward, actor.distances, actor.delay, actor.constrained_delivery_time, actor.attending, actor.pointing], feed_dict=feed)
                ...
                else: predictions_length.append(training_set.get_tour_length(or_sequence[best_permutation])/100)
                predictions_delay.append(delay[j])

                # Improve tour with 2 opt
                two_opt_input = np.concatenate((or_sequence[best_permutation], tw_open[best_permutation], tw_close[best_permutation]), axis=1)
